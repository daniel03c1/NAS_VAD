...
class GraphGP:
    ...
    def predict(self, X_s, preserve_comp_graph=False):
        ...
        K_full = sum_kernel_copy.fit_transform(
            self.weights, X_all, X_features_all, self.theta_vector,
    ...
    def dmu_dphi(self, X_s=None,
        ...
        """
        if self.K_i is None or self.logDetK is None:
            raise ValueError("Inverse of Gram matrix is not instantiated. "
                             "Please call the optimize function to "
                             "fit on the training data first!")
        ...
        for j, x_s in enumerate(X_s):
            ...
            # Each handle is a 2-tuple. first element is the Gram matrix,
            # second element is the leaf variable
            if V_s is None:
            ...
            for handle in handles:
                k_s, y, _ = handle
                # k_s is output, leaf is input, alpha is the K_i @ y term
                # which is constant.
                jacob_vecs.append(
                    torch.autograd.grad(outputs=k_s, inputs=y, grad_outputs=alpha,
        ...
        if average_across_features:
            ...
            avg_mu, avg_var, incidences = get_grad(dmu_dphi, feature_matrix,
                                                   average_across_occurrences)
            return avg_mu, avg_var, incidences

        if average_across_occurrences:
            return dmu_dphi, None, feature_matrix.sum(dim=0)
        else:
...
def get_grad(grad_matrix, feature_matrix, average_occurrences=False):
    ...
    if average_occurrences:
        ...
        for d in range(D):
            ...
            weight_vector = (current_feature > 0).to(torch.float32) # fixed
        ...
        # The maximum number possible occurrences -- 7 is an example,
        # if problem occurs, maybe we can increase this number.
        # But for now, for both NAS-Bench datasets, this should be more than enough!
        max_occur = 7
