...
class Interpreter:
    def __init__(self, X=None, y=None, h=1, gp=None, thres=25, occur_thres=10):
        ...
        feat_list = self.gp.kernels[0].feature_value(self.X_)[1]
    ...
    def explain(self, X_s, y_s, percentile):
        ...
        embedding = self.gp.kernels[0].feature_value([X_s])[0].flatten()
    ...
    def validate(self, X_s, y_s):
        ...
        for i, x_s in tqdm(enumerate(X_s)):
            for g in good_features:
                if g.count('~') == 0:
                    continue
                if find_wl_feature(x_s, (g,), self.gp.kernels[0]):
                    good_perf.append(y_s[i])
                    break
            for b in bad_features:
                if b.count('~') == 0:
                    continue
                if find_wl_feature(x_s, (b,), self.gp.kernels[0]):
