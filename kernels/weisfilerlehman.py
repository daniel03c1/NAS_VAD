...
class WeisfilerLehman(GraphKernels):
    ...
    def __init__(self, h: int = 0,
        ...
        kwargs
        """
        super(WeisfilerLehman, self).__init__(**kwargs)
        ...
        self.type = type
        self.kern = _WL(n_jobs, h=h, base_graph_kernel=base_kernel, normalize=True,
                        layer_weights=self.layer_weights, node_weights=node_weights)
    ...
    def fit_transform(self, gr: list, rebuild_model=False,
                      save_gram_matrix=True, layer_weights=None, **kwargs):
        ...
        if layer_weights is not None \
                and layer_weights is not self.layer_weights:
    ...
    def forward_t(self, gr2, gr1=None):
        ...
        """
        Note that the vector length of the WL procedure is indeterminate, and thus
        dim(Y) != dim(X) in general.
        However, since the newly observed features in the test data is always
        concatenated at the end of the feature matrix, these features will not
        matter for the inference, and as such we can safely truncate the feature
        matrix for the test data so that only those appearing in both the training 
        and testing datasets are included.
        """
